...
test('internal extension', t => {
    ...
    const _blockInfo = vm.runtime._blockInfo[vm.runtime._blockInfo.length - 1];
    t.equal(_blockInfo.menus.length, 2);
    // First menu has 3 items.
    t.equal(
        _blockInfo.menus[0].json.args0[0].options.length, 3);
    // Second menu is a dynamic menu and therefore should be a function.
    t.type(
        _blockInfo.menus[1].json.args0[0].options, 'function');
...
test('load sync', t => {
    const vm = new VirtualMachine();
    const prevBlockInfoLength = vm.runtime._blockInfo.length;
    vm.extensionManager.loadExtensionIdSync('coreExample');
    t.ok(vm.extensionManager.isExtensionLoaded('coreExample'));

    t.equal(vm.runtime._blockInfo.length, prevBlockInfoLength + 1);

    // blocks should be an array of two items: a button pseudo-block and a reporter block.
    const _blockInfo = vm.runtime._blockInfo[vm.runtime._blockInfo.length - 1];
    t.equal(_blockInfo.blocks.length, 3);
    t.type(_blockInfo.blocks[0].info, 'object');
    t.type(_blockInfo.blocks[0].info.func, 'MAKE_A_VARIABLE');
    t.equal(_blockInfo.blocks[0].info.blockType, 'button');
    t.type(_blockInfo.blocks[1].info, 'object');
    t.equal(_blockInfo.blocks[1].info.opcode, 'exampleOpcode');
    t.equal(_blockInfo.blocks[1].info.blockType, 'reporter');
    t.type(_blockInfo.blocks[2].info, 'object');
    t.equal(_blockInfo.blocks[2].info.opcode, 'exampleWithInlineImage');
    t.equal(_blockInfo.blocks[2].info.blockType, 'command');

    // Test the opcode function
    t.equal(_blockInfo.blocks[1].info.func(), 'no stage yet');
    ...
    t.equal(_blockInfo.blocks[1].info.func(), 'Stage');
